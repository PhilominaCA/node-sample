...
app.delete("/movies/:id", async(req,resp)=>{ //102 deleted
  const {id} =req.params;
 const result = await client.db("b251we").collection("movies").deleteMany({id:id});
 resp.send(result);
});

app.put("/movies/:id", async(req,resp)=>{ //102 deleted
  const {id} =req.params;
...
// console.log(result);     console.log(req.query);     console.log(language,rating);
...
app.get("/movies/:id", async(req,resp)=>{
  ...
  //mongo command eg: db.moviesfindOne({"id":"105"})
   const result = await client.db("b251we").collection("movies").findOne({id:id});
   result? resp.send(result):resp.status(404).send({msg:"MOVIE NOT FOUND"});
  // resp.send(movies.find((movie)=>movie.id==id))
});

app.get("/movies",(req,resp)=>{
    const {language,rating} = req.query;
  const langFilter =  language?movies.filter((movie)=>movie.language === language):movies;
    const result = rating?(langFilter.length?langFilter.filter((movie)=>movie.rating===+rating):movies.filter((movie)=>movie.rating===+rating)):(langFilter.length?langFilter:movies);
    result.length?resp.send(result):resp.send(movies);
});
*/
/**app.get("/movies",(req,resp)=>{
  const {language,rating} = req.query;
